  ...
  methods: {
    async fetch() {
      ...
      const [downloads, stars, contributors] = await Promise.all([
        fetch('https://api.npmjs.org/downloads/range/2013-08-21:2100-08-21/telegram.js').then(json, noop),
        fetch('https://api.github.com/repos/yuvapoojary/telegram.js').then(json, noop),
        fetch('https://api.github.com/repos/yuvapoojary/telegram.js/stats/contributors').then(json, noop),
      ]);
